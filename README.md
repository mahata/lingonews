...
- **Pre-commit** — runs RuboCop (on staged `.rb` files) and a full-project TypeScript type check in parallel. RuboCop only inspects the files you staged; the TypeScript check triggers when any `.ts`/`.tsx` file is staged but runs the full project since `tsc` requires whole-program context. This catches style violations and type errors before they reach CI.
